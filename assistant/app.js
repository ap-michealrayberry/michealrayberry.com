...
(function (MRB) {
  ...
  async function runDaily(session) {
    // Official sequence: WAIT → FRONT → LEFT → REAR → RIGHT → WAIT
    var views = [
      "wait_open",
      "inspection",
      "left",
  ...
  async function runCorner(session) {
    ...
    await speakAndHold(session, MRB.scripts.cornerTimerComplete(session), 4);
    if (session.aborted) return;
  ...
  async function runWeekly(session) {
    ...
    await speakAndHold(session, MRB.scripts.weeklyOpening(session), 10);
    if (session.aborted) return;

    MRB.ui.setStatus("session", "The record");
    for (var i = 0; i < (fig.lines || []).length; i++) {
      if (session.aborted) return;
      await speakAndHold(session, fig.lines[i], 5);
    }
