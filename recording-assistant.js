    ...
    <div class="ra-controls">
      ...
      <div class="ra-mealbox" style="margin-top:18px;padding-top:16px;border-top:1px solid #D8D6CF">
        ...
        <p class="ra-mealnote" style="margin-top:8px">
          Optional, independent of the inspection above. Point the camera at the meal as served, before eating, and tap once. Stamped with the day number and date, saved at full resolution. No weight entry and no uniform required.
        </p>
      </div>
  ...
  class RecordingAssistant extends HTMLElement {
    connectedCallback() {
      ...
      async function fetchChallenge(kind) {
        try {
          const r = await fetch(ATTEST_ENDPOINT + '?action=challenge&kind=' + kind);
      ...
      async function attestPost(payload) {
        try {
          const r = await fetch(ATTEST_ENDPOINT, { method: 'POST', body: JSON.stringify(Object.assign({ action: 'attest', date: isoStr, day: dayNum }, payload)) });
          const j = await r.json();
          return !!(j && j.ok);
      ...
      function captureMeal() {
        ...
        p.fillStyle = '#FF6B61'; p.font = '600 ' + Math.round(24 * k) + 'px "IBM Plex Mono",monospace';
        p.fillText('PUBLIC ACCOUNTABILITY PROJECT', Math.round(40 * k), Math.round(94 * k));
        const tag = 'EVENING MEAL';
        p.font = '700 ' + Math.round(34 * k) + 'px "IBM Plex Mono",monospace'; p.fillStyle = '#FAFAF7';
        p.fillText(tag, PW - p.measureText(tag).width - Math.round(40 * k), Math.round(62 * k));
        p.fillStyle = 'rgba(15,15,13,0.82)'; p.fillRect(0, PH - Math.round(128 * k), PW, Math.round(128 * k));
        p.fillStyle = '#FAFAF7'; p.font = '600 ' + Math.round(34 * k) + 'px "IBM Plex Mono",monospace';
        p.fillText(dayLabel + ' — ' + isoStr, Math.round(40 * k), PH - Math.round(82 * k));
        p.fillStyle = '#B9B8B2'; p.font = '400 ' + Math.round(24 * k) + 'px "IBM Plex Mono",monospace';
        p.fillText('EVENING MEAL · AS SERVED · MICHEALRAYBERRY.COM', Math.round(40 * k), PH - Math.round(40 * k));
        const dl = $('.ra-mealdl');
      ...
      $('.ra-cstart').addEventListener('click', async () => {
        ...
        const startLine = 'Corrective session. Level ' + level + '. ' + mins + ' minutes, continuous, single take. Verification code: ' + ch.code.split('').join(', ') + '. Assume the position: standing, facing the camera, hands behind the head. Position, movement, and room audio are monitored. Ten seconds to take position. The session begins now.';
        const markLines = [];
