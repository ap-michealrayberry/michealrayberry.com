/* Cloudflare Pages Function — POST /observer
   Receives the Observer Submission form, verifies Cloudflare Turnstile, then
   relays the fields server-side to the record's Apps Script (action
   'observer') with a shared secret. Nothing is stored here.
   Env (Pages → Settings → Variables & Secrets):
     TURNSTILE_SECRET   Turnstile widget secret key
     OBSERVER_SECRET    same value as setObserverSecret() in Code.gs
     APPS_SCRIPT_URL    the /exec URL of the record script */
export async function onRequestPost({ request, env }) {
  ...
  if (String(form.get('website') || '').trim()) return back('error=spam'); // honeypot
