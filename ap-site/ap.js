...
(function () {
  ...
  /* Same-origin relay. Cloudflare Access authenticated the AP at the edge;
     the relay re-verifies the identity token and holds the AP key in
     secrets. Nothing is stored on this device. */
  async function api(op, extra) {
    var body = Object.assign({ op: op }, extra || {});
    var r = await fetch("/api/ap", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(body) });
    if (r.status === 401) { location.reload(); throw new Error("Session expired — sign in again."); }
  ...
  async function act(op, extra, o) {
    ...
    try { await api(op, Object.assign({}, extra || {}, o ? { confirmed: true } : {})); toast(o && o.done || "Done"); await load(); }
  ...
  /* ── wiring ── */
  if ($("lock-form")) $("lock-form").addEventListener("submit", function (e) { e.preventDefault(); load(); });
